...
function Edit({ auth, customer }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Ubah Data Customer</h2>}
        >
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                       <form onSubmit={handleSubmit}>
                           ...
                           <div className="mb-6">
                               <label htmlFor="phone" className="block mb-2 text-sm font-medium text-gray-900">Foto ID Card (PNG/JPEG)</label>
                               <input type="file" name="id_card" accept="image/png, image/jpeg"
                                      onChange={(e) => setIdCard(e.target.files[0])}
                                      className="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-400 focus:border-indigo-400 block w-full p-2.5 placeholder-gray-400"
                                      placeholder="082138172391" />
                           </div>
                           <button type="submit"
